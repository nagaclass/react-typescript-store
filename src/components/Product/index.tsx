...
const Product: React.FC<Props> = ({ product, addToCartHandler }) => {
  ...
  return (
    ...
    >
      ...
      <div>
        <h6 onClick={() => addToCartHandler(product)} className="text-sm mt-10 font-semibold">
          {textSlicer(title, 30)}
        </h6>
        <p className="text-sm mt-5 font-semibold">{textSlicer(description, 100)}</p>
        <h6 className="font-semibold mt-5">${price}</h6>
        <button className="bg-slate-700 w-full text-white py-2 rounded-md cursor-pointer mt-5 uppercase font-bold text-sm">
          Add to Cart
        </button>
      </div>
    </div>
